...
The app then **paints the look onto your photo**, with a before/after wipe and an intensity slider.

What decides whether that reads as makeup or as a sticker is where the shapes come from. **Lipstick and brows are not drawn — they are cut out of your own photograph.** Lips are redder than the skin around them and brows are darker, so the renderer measures both against the ring of skin just outside the region and tints only the pixels that stand out. The lipstick then follows *your* lip line, keeps the line between your lips readable, and leaves the highlight exactly where the light fell. The brow colour lands on the brow that is already there instead of floating above it as an arc. The reference is local rather than a fixed threshold, so it survives a face lit from one side, any skin depth and any white balance — and when a feature genuinely can't be told apart from its surroundings, the drawn shape takes over rather than nothing appearing.

Everything else is shaped the way it is worn. Eyeshadow is a run of soft dabs along the lash line, weighted to the outer third and lifting into the crease — never a filled shape, because a shape has an edge and an edge is the tell. Eyeliner is a thin tapered stroke that hugs the lash line, with a wing when the look calls for one. Mascara is short strokes rather than a smear. Foundation evens the skin with a softened copy of itself before any colour goes on.
...
**Or let the app run it.** Add a Google AI key in settings and the brief screen grows a **Render it now** button: the same instruction and the same photographs go from your browser straight to Google's image model (Gemini 3.1 Flash Image, falling back to 2.5 where unavailable), and the picture that comes back is saved onto the look automatically. Same architecture as the Claude key — your key, your device, no server of ours in between. Costs about $0.07 a picture and needs billing enabled on the Google account (image generation is not in the API free tier); the manual brief stays as the free route.
